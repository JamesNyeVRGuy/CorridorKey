<script lang="ts">
  ...
  async function refreshCredits() {
    try {
      ...
      const orgId = getOrg();
      if (orgId) {
        const credits = await fetch(`/api/orgs/${orgId}/credits`, {
          headers,
        }).then((r) => r.json());
        ...
        {#if authEnabled && $userOrgs.length > 1}
          <div class="org-switcher">
            <select
              ...
              value={$activeOrgId}
              onchange={(e) =>
                handleOrgSwitch((e.target as HTMLSelectElement).value)}
            ...
            >
              <span class="nav-icon">
                {#if item.icon === "film"}
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                    ><rect
                      ...
                      stroke-width="1.2"
                    /><path
                      d="M5 3v10M11 3v10M2 6.5h3M11 6.5h3M2 9.5h3M11 9.5h3"
                      stroke="currentColor"
                      stroke-width="1.0"
                    /></svg
                  >
                {:else if item.icon === "layers"}
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                    ><path
                      d="M2 8l6 3.5L14 8M2 10.5l6 3.5 6-3.5M2 5.5L8 9l6-3.5L8 2 2 5.5z"
                      stroke="currentColor"
                      stroke-width="1.2"
                ...
                {:else if item.icon === "server"}
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                    ...
                    /><circle
                      ...
                      r="0.8"
                      fill="currentColor"
                    /><circle cx="5" cy="12" r="0.8" fill="currentColor" /></svg
                  >
      ...
      <div class="sidebar-bottom">
        {#if !authEnabled || getStoredUser()?.tier === "platform_admin"}
          ...
          <div class="device-row">
            ...
            <span class="conn-badge mono" class:live={connected}
        ...
        {:else}
          {#if creditBalance}
            <div
              ...
              class:positive={creditBalance.positive}
              class:negative={!creditBalance.positive}
            >
              <span class="credit-icon mono">GPU</span>
              <span class="credit-amt mono"
                >{creditBalance.positive ? "+" : ""}{creditBalance.hours}h</span
              >
            </div>
          {/if}
          <div class="device-row">
        ...
        <div class="external-links">
          <a
            ...
            target="_blank"
            rel="noopener"
            class="external-link mono"
          >
            <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"
              ><path
                d="M13.5 3.2A12.3 12.3 0 0 0 10.3 2a8 8 0 0 0-.4.8 11.5 11.5 0 0 0-3.8 0A8 8 0 0 0 5.7 2 12.3 12.3 0 0 0 2.5 3.2 13.7 13.7 0 0 0 .1 12.5a12.4 12.4 0 0 0 3.8 2 9 9 0 0 0 .8-1.3 8 8 0 0 1-1.3-.6l.3-.2a8.8 8.8 0 0 0 7.6 0l.3.2a8 8 0 0 1-1.3.6 9 9 0 0 0 .8 1.3 12.4 12.4 0 0 0 3.8-2A13.7 13.7 0 0 0 13.5 3.2ZM5.3 10.7c-.8 0-1.5-.8-1.5-1.7s.7-1.7 1.5-1.7 1.5.8 1.5 1.7-.7 1.7-1.5 1.7Zm5.4 0c-.8 0-1.5-.8-1.5-1.7s.7-1.7 1.5-1.7 1.5.8 1.5 1.7-.7 1.7-1.5 1.7Z"
              /></svg
            >
            Discord
          </a>
          <a
            ...
            target="_blank"
            rel="noopener"
            class="external-link support-link mono"
          >
            <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"
              ><path
                d="M2 4.5A1.5 1.5 0 0 1 3.5 3h7A1.5 1.5 0 0 1 12 4.5v.5h.5A1.5 1.5 0 0 1 14 6.5v1A1.5 1.5 0 0 1 12.5 9H12v.5A1.5 1.5 0 0 1 10.5 11h-7A1.5 1.5 0 0 1 2 9.5v-5Zm10 1v3h.5a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5H12ZM3 12.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5Z"
              /></svg
            >
            Support
          </a>
        ...
        {#if authEnabled}
          ...
          <div class="user-row">
            <a href="/profile" class="user-email mono"
              >{user?.name || user?.email || ""}</a
            >
            <button class="logout-btn mono" onclick={handleLogout}
              >LOGOUT</button
            >
          </div>
        {/if}
        ...
        <div class="activity-bar">
          <div class="activity-info mono">
            ...
            {#if rJob.total_frames > 0 && rJob.current_frame >= rJob.total_frames}
              <span class="activity-pct uploading"
                >Uploading{rJob.upload_pass
                  ? ` ${rJob.upload_pass.toUpperCase()}`
                  : ""}...</span
              >
            {:else if rJob.total_frames > 0}
              <span class="activity-pct"
                >{Math.round(
                  (rJob.current_frame / rJob.total_frames) * 100,
                )}%</span
              >
            {/if}
          </div>
          <div class="activity-track">
            <div
              class="activity-fill"
              style="width: {rJob.total_frames > 0
                ? (rJob.current_frame / rJob.total_frames) * 100
                : 0}%"
            ></div>
          </div>
...
<style>
  ...
  .credit-indicator.positive {
    background: rgba(93, 216, 121, 0.08);
  }
  .credit-indicator.negative {
  ...
  .credit-indicator.positive .credit-amt {
    color: var(--state-complete);
  }
  .credit-indicator.negative .credit-amt {
    color: var(--state-error);
  }
  ...
  .external-link.support-link:hover {
    color: var(--accent);
